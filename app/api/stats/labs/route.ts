...
export async function GET() {
  try {
    ...
    const q = query(collection(db, "prescriptions"), orderBy("uploadedAt", "desc"));
      ...
      for (const key of Object.keys(values)) {
        for (const lab of interestingLabs) {
          if (key.toLowerCase().includes(lab.toLowerCase())) {
            ...
            if (parsed !== null && !Number.isNaN(parsed)) stats[lab].values.push(parsed);
          }
        }
      }
    });
    ...
    return NextResponse.json({ error: "Failed to compute lab stats" }, { status: 500 });
  }
}
